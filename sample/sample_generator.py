...
'''
    sample format
    line_id \001 0:label0;1:label1,.... \001 slot_id \001 fea_attr \002 fea_attr \002 ....
...
class Sample:
    ...
    def format(self, lines: list):
        ...
        fea_str = [self.SPLIT_2.join(['{}:{}'.format(k, l) for l in v]) for k, v in self.feature.items()]
        dense_fea_str = ['{}:{}'.format(k, ','.join(v)) for k, v in self.dense_feature.items()]
        return self.SPLIT_1.join([self.line_id, label_str, self.SPLIT_1.join(fea_str), dense_fea_str])
